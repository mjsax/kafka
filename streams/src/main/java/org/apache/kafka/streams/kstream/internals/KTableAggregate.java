...
public class KTableAggregate<K, V, T> implements KTableProcessorSupplier<K, V, T> {
    ...
    private class KTableAggregateProcessor extends AbstractProcessor<K, Change<V>> {
        ...
        public void process(final K key, final Change<V> value) {
            ...
            final ValueAndTimestamp<T> oldAgg = store.get(key);
            final T plainOldAgg;
            final long resultTimestamp;
            if (oldAgg != null) {
                plainOldAgg = oldAgg.value();
                resultTimestamp = Math.max(oldAgg.timestamp(), context().timestamp());
            } else {
                plainOldAgg = initializer.apply();
                resultTimestamp = context().timestamp();
            }

            T newAgg = plainOldAgg;
            ...
            tupleForwarder.maybeForward(key, newAgg, plainOldAgg, resultTimestamp);
        }
