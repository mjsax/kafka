...
class KTableProcessValues<K, V, VOut> implements KTableProcessorSupplier<K, V, K, VOut> {
    ...
    private final FixedKeyProcessorSupplier<? super K, ? super V, ? extends VOut> processorSupplier;
    ...
    public Processor<K, Change<V>, K, Change<VOut>> get() {
        return new KTableProcessValuesProcessor(processorSupplier.get());
    }
    ...
    private class KTableProcessValuesProcessor extends ContextualProcessor<K, Change<V>, K, Change<VOut>> {
        private final FixedKeyProcessor<? super K, ? super V, ? extends VOut> valueProcessor;
        private KeyValueStoreWrapper<K, VOut> store;
        private TimestampedTupleForwarder<K, VOut> tupleForwarder;

        private KTableProcessValuesProcessor(final FixedKeyProcessor<? super K, ? super V, ? extends VOut> valueProcessor) {
        ...
        public void init(final ProcessorContext<K, Change<VOut>> context) {
            ...
            valueProcessor.init(new ForwardingDisabledProcessorContext(internalProcessorContext));
            if (queryableName != null) {
        ...
        public void process(final Record<K, Change<V>> record) {
            valueProcessor.process(
                InternalFixedKeyRecordFactory.create(
                    record.withValue(record.value().newValue)
                )
            );
            ...
            if (queryableName != null) {
                ...
                final VOut oldValue = sendOldValues ? valueProcessor.process(record.key(), record.value().oldValue); : null;
