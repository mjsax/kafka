...
public class KGroupedTableImplTest {
    ...
    public void shouldNotAllowInvalidStoreNameOnReduce() {
        groupedTable.reduce(MockReducer.STRING_ADDER, MockReducer.STRING_REMOVER, Materialized.as(INVALID_STORE_NAME));
    }
    ...
    private Map<String, Integer> getReducedResults(final KTable<String, Integer> inputKTable) {
        ...
        inputKTable.toStream().foreach(reducedResults::put);
    ...
    public void shouldReduceAndMaterializeResults() {
        ...
        try (final TopologyTestDriver driver = new TopologyTestDriver(builder.build(), props)) {
            assertReduced(results, topic, driver);
            final KeyValueStore<String, ValueAndTimestamp<Integer>> reduce = driver.getKeyValueStore("reduce");
            assertThat(reduce.get("A"), equalTo(new ValueAndTimestampImpl<>(5, 10L)));
            assertThat(reduce.get("B"), equalTo(new ValueAndTimestampImpl<>(6, 10L)));
        }
    }
    ...
    public void shouldCountAndMaterializeResults() {
        ...
        try (final TopologyTestDriver driver = new TopologyTestDriver(builder.build(), props)) {
            ...
            final KeyValueStore<String, ValueAndTimestamp<Long>> counts = driver.getKeyValueStore("count");
            assertThat(counts.get("1"), equalTo(new ValueAndTimestampImpl<>(3L, 5L)));
            assertThat(counts.get("2"), equalTo(new ValueAndTimestampImpl<>(2L, 6L)));
        }
    }
    ...
    public void shouldAggregateAndMaterializeResults() {
        ...
        try (final TopologyTestDriver driver = new TopologyTestDriver(builder.build(), props)) {
            ...
            assertThat(aggregate.get("1"), equalTo(new ValueAndTimestampImpl<>("0+1+1+1", 5L)));
            assertThat(aggregate.get("2"), equalTo(new ValueAndTimestampImpl<>("0+2+2", 6L)));
        }
    }
    ...
    private void processData(final String topic, final TopologyTestDriver driver) {
        ...
        driver.pipeInput(recordFactory.create(topic, "A", "1", 1L));
        driver.pipeInput(recordFactory.create(topic, "B", "1", 5L));
        driver.pipeInput(recordFactory.create(topic, "C", "1", 3L));
        driver.pipeInput(recordFactory.create(topic, "D", "2", 4L));
        driver.pipeInput(recordFactory.create(topic, "E", "2", 6L));
    }
}
